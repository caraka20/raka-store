...
const AddProduct = () => {
  ...
  const [dataInputKategori, setDataInputKategori] = useState(
    Number(new URLSearchParams(param.search).get("kategori"))
  );
  const kategoriId = Number(new URLSearchParams(param.search).get("kategori"));
  const kategori = dataKategori.find((kategori) => kategori.id === kategoriId);
  const lala = kategori ? kategori.nama_kategori : "Tidak ada";
  ...
  const handleKategori = (e) => {
    window.location.href = `/dashboard/add-product?kategori=${Number(
      e.target.value
    )}`;
  };
